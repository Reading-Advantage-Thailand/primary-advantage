...
export const generateExportWorkbook = async (articleId: string) => {
  try {
    ...
    const workbookData: WorkbookJSON = {
      ...
      vocabulary: vocab,
      article_image_url: [
        `https://storage.googleapis.com/primary-app-storage/images/${article.id}_1.png`,
        `https://storage.googleapis.com/primary-app-storage/images/${article.id}_2.png`,
        `https://storage.googleapis.com/primary-app-storage/images/${article.id}_3.png`,
      ],
      article_caption: article.summary || "Article Illustration",
      article_url: `https://primary.reading-advantage.com/student/read/${article.id}`,
