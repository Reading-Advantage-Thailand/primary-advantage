...
class DashboardTelemetryService {
  ...
  constructor() {
    ...
    if (typeof window !== "undefined") {
      ...
      if (
        ...
        window.localStorage.getItem("analytics-disabled") === "true"
      ) {
  ...
  private track(
    event: Omit<TelemetryEvent, "timestamp" | "sessionId" | "userId">,
  ): void {
  ...
  public heatmapView = (
    properties: Omit<HeatmapInteraction["properties"], "interactionType">,
  ): void => {
    this.track({
  ...
  public timelineView = (
    properties: Omit<TimelineInteraction["properties"], "interactionType">,
  ): void => {
    this.track({
  ...
  public dashboardNavigation = (
    properties: DashboardNavigation["properties"],
  ): void => {
    this.track({
