...
export const articleGeneratorSchema = z.object({
  ...
  passage: z
    .string()
    .describe(
      `A reading passage written to the specified CEFR level and type.
...
export const contactMessageListQuerySchema = z.object({
  ...
  status: z.enum(["NEW", "READ", "REPLIED", "ARCHIVED", "ALL"]).default("ALL"),
  inquiry: z.enum(["SALES", "SUPPORT", "PARTNERSHIPS", "ALL"]).default("ALL"),
...
// List query schemas for teacher and classroom endpoints
...
// Update teacher body schema (PUT /api/teachers/[id])
export const updateTeacherInputSchema = z.object({
  ...
  schoolId: z.string().cuid().optional(),
  assignedClassroomIds: z.array(z.string().cuid()).optional(),
});

// Update classroom body schema (PUT /api/classrooms/[id])
export const updateClassroomInputSchema = z.object({
  name: z.string().min(1).optional(),
  grade: z.string().optional(),
  passwordStudents: z.string().optional(),
  assignedTeacherIds: z.array(z.string().cuid()).optional(),
});
