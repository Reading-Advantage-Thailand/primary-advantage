...
import { runArticleValidation } from "@/server/controllers/articleValidationController";
...
export async function POST(req: NextRequest) {
  ...
  return withAuth(
    async (authReq, _ctx, user) => {
      console.log(`[Article Validate] Manual trigger by system user ${user.id}`);
...
async function handle(
  ...
  try {
    const { summary, conflict } = await runArticleValidation({
      mode: parsed.mode,
      ids: parsed.ids,
      limit: parsed.limit,
    ...
    return NextResponse.json(
      {
        message: "Article validation completed",
        source,
    ...
    console.error("[Article Validate] Unexpected error:", err);
