...
import {
  ...
  Difficulty,
} from "@/lib/games/enchanted-library/enchantedLibrary";
import type { VocabularyItem } from "@/store/useGameStore";
import { useSound } from "@/hooks/useSound";
import { useInterval } from "@/hooks/useInterval";
import { useDirectionalInput } from "@/hooks/useDirectionalInput";
import { VirtualDPad } from "@/components/games/VirtualDPad";
...
export function EnchantedLibraryGame({
    ...
    const load = async () => {
      ...
      try {
        const [player, spirit, book, floor] = await Promise.all([
          ...
          loadImage("/games/enchanted-library/library_background.png"),
        ]);
  ...
  if (!assets) {
    return (
      <div
        ref={containerRef}
        className="relative flex h-[50vh] w-full items-center justify-center overflow-hidden rounded-2xl border border-amber-300 bg-linear-to-b from-amber-100 to-amber-200 sm:h-[60vh] md:aspect-video md:h-auto"
      >
        <div className="animate-pulse font-mono text-sm tracking-widest text-amber-800 uppercase sm:text-base">
          Loading Library...
        </div>
      </div>
    );
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-[80vh] max-h-190 min-h-80 w-full touch-none overflow-hidden rounded-3xl bg-slate-950 shadow-2xl"
    >
      {gamePhase === "start" && (
        <GameStartScreen
          gameTitle={t("title")}
      ...
      {gamePhase === "playing" && gameState && grids && (
        ...
        >
          {/* HUD Overlay */}
          <div className="pointer-events-none absolute top-2 left-2 z-10 flex flex-col gap-1 text-sm font-bold text-amber-900 drop-shadow-[0_2px_4px_rgba(255,255,255,0.8)] sm:top-4 sm:left-4 sm:text-base md:text-xl">
            <div className="rounded-lg bg-white/80 px-2 py-0.5 sm:px-3 sm:py-1">
              {t("hud.mana")}: {gameState.mana}
            </div>
            <div
              className={`rounded-lg px-2 py-0.5 sm:px-3 sm:py-1 ${
            ...
            <div className="flex items-center gap-0.5 rounded-lg bg-white/80 px-2 py-0.5 text-sm text-blue-600 sm:gap-1 sm:px-3 sm:py-1 sm:text-base">
              {t("hud.shields")}:{" "}
          ...
          <div className="pointer-events-none absolute top-2 left-1/2 z-10 max-w-[55%] -translate-x-1/2 rounded-full border-2 border-yellow-300 bg-linear-to-r from-yellow-400 to-amber-500 px-3 py-1.5 text-center shadow-lg backdrop-blur-sm sm:top-4 sm:max-w-none sm:px-6 sm:py-3">
            <span className="mr-1 text-xs text-white/90 sm:mr-2 sm:text-sm">
              Find:
            </span>
            <span className="text-base font-bold text-white drop-shadow-md sm:text-xl md:text-2xl">
          ...
          <div className="absolute top-2 right-2 z-20 sm:top-4 sm:right-4">
            <button
              ...
              className="rounded-full border-2 border-amber-300 bg-white/90 p-2 text-amber-900 shadow-lg transition-transform hover:scale-110 active:scale-95 sm:p-3"
              title="My Grimoire"
            >
              <Book className="h-4 w-4 sm:h-6 sm:w-6" />
            </button>
          </div>
          ...
          {/* Virtual Controls */}
          <div className="absolute right-3 bottom-3 z-20 flex flex-row items-end gap-3 sm:right-8 sm:bottom-8 sm:gap-6">
            <button
              onClick={() => triggerCast()}
              disabled={gameState.player.shieldCharges === 0}
              className={`flex h-14 w-14 items-center justify-center rounded-full border-2 text-xs font-bold transition-all active:scale-95 sm:h-20 sm:w-20 sm:text-sm ${
                gameState.player.shieldCharges > 0
                  ? "border-blue-400 bg-blue-600 text-white shadow-[0_0_15px_rgba(59,130,246,0.5)]"
                  : "border-slate-300 bg-slate-400 text-slate-600 opacity-50"
              }`}
            >
              SHIELD
            </button>
