...
import type { RPGBattleCompleteResult } from "./RPGBattle";

export type Difficulty = "slime" | "goblin" | "spectre" | "elemental";
...
export default function StartScreenDragonRider() {
  ...
  const { rankings: rawRankings, isLoading: isLoadingRankings } =
    useRPGBattleRanking();
  const { submitResult } = useSubmitRPGBattleResult();

  const handleComplete = useCallback(
    async (results: RPGBattleCompleteResult) => {
      try {
        await submitResult({
          xp: results.xp,
          ...
          turnsTaken: results.turnsTaken,
          heroId: results.heroId,
          enemyId: results.enemyId,
          outcome: results.outcome,
        });
      } catch (err) {
        console.error("Failed to submit RPG Battle result:", err);
      }
    },
    [submitResult],
  );
  ...
  const rankings = {
    slime: rawRankings.slime ?? [],
    goblin: rawRankings.goblin ?? [],
    spectre: rawRankings.spectre ?? [],
    elemental: rawRankings.elemental ?? [],
  };
      ...
      {activeTab === "game" ? (
        ...
        <div className="w-full rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900/50">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            {(["slime", "goblin", "spectre", "elemental"] as Difficulty[]).map(
              (dif) => (
                <div
                  key={dif}
