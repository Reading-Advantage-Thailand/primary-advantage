      ...
      <AnimatePresence>
        {items.map((item) => (
          <motion.div
            ...
            className={cn(
              "absolute text-2xl font-black whitespace-nowrap drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] select-none",
              item.type === "damage-player" && "text-red-500",
              item.type === "damage-enemy" && "stroke-black text-white",
              item.type === "heal" && "text-green-400",
              item.type === "crit" && "text-3xl text-yellow-400",
            )}
          >
            {item.text}
          </motion.div>
