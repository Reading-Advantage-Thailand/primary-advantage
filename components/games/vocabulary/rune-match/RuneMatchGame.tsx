...
import {
  ...
  findPossibleMoves,
  type RuneMatchState,
} from "@/lib/games/rune-match/runeMatch";
import {
  RUNE_MATCH_CONFIG,
  type MonsterType,
...
export function RuneMatchGame({ vocabulary, onComplete }: RuneMatchGameProps) {
  ...
  const layout = useMemo(() => {
    const padding = 16;
    const isMobile = dimensions.width <= 640; // Mobile breakpoint
  ...
  if (!assets || !gameState || dimensions.width === 0) {
    return (
      <div
        ...
        className="relative flex h-[60vh] w-full items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-slate-950 md:aspect-video md:h-auto"
      >
  ...
  const calcFontSize = (text: string, cellSize: number): number => {
    const isThai = isThaiText(text);
    // Thai glyphs are wider visually; English words can split at spaces
    const factor = isThai ? 0.5 : 0.65;
    const runeDrawSize = cellSize - 4;
    const maxByCell = cellSize / (text.length > 8 ? 3.5 : 3);
    const fitByLength = (runeDrawSize - 10) / Math.max(1, text.length * factor);
    const minSize = cellSize < 60 ? 9 : 10;
    return Math.max(minSize, Math.min(maxByCell, fitByLength));
  };
  ...
  return (
    <div
      ref={containerRef}
      data-testid="rune-match-container"
      className="relative h-[80vh] w-full overflow-hidden rounded-2xl border border-white/10 bg-slate-900/40 backdrop-blur-sm md:aspect-video md:h-auto"
    >
      <AnimatePresence mode="wait">
        {gameState.status === "selection" && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-10 flex items-start justify-center overflow-y-auto bg-slate-950/80 backdrop-blur-sm"
          >
        ...
        {gameState.status === "victory" && (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="absolute inset-0 z-20 flex flex-col items-center justify-center space-y-6 bg-slate-950/90 text-center backdrop-blur-md"
          >
            <div className="flex h-35 w-35 items-center justify-center overflow-hidden rounded-3xl border-2 border-yellow-500/30 bg-yellow-500/20 shadow-lg shadow-yellow-500/10">
              <div
                style={{
                  width: "120px",
                  height: "120px",
            ...
            <div className="min-w-50 rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <Button
              ...
              size="lg"
              className="bg-yellow-500 px-12 font-bold text-black hover:bg-yellow-600"
            >
              {t("common.continue")}
            </Button>
          </motion.div>
        ...
        {gameState.status === "defeat" && (
          <motion.div
            ...
            className="absolute inset-0 z-20 flex flex-col items-center justify-center space-y-6 bg-slate-950/90 text-center backdrop-blur-md"
          >
            <div className="flex h-35 w-35 items-center justify-center overflow-hidden rounded-3xl border-2 border-red-500/30 bg-red-500/20 shadow-lg shadow-red-500/10">
              <div
                style={{
                  width: "120px",
                  height: "120px",
            ...
            <Button
              ...
              variant="outline"
              size="lg"
              className="border-red-500 px-12 text-red-500 hover:bg-red-500 hover:text-white"
            >
              {t("common.tryAgain")}
            </Button>
          </motion.div>
                ...
                {!layout.isMobile && (
                  <Group>
                    ...
                    {gameState.monster && (
                      <Group>
                        ...
                        {renderHealthBar(
                          ...
                          "#ef4444",
                          t(`runeMatch.monsters.${gameState.monster.type}`),
                          // gameState.monster.type.toUpperCase(),
                        )}
                      </Group>
                    )}
                    <Text
                      text={t("runeMatch.inGame.powerWord")}
                      x={10}
                      y={140}
                    ...
                    {renderHealthBar(
                      ...
                      "#22c55e",
                      t("runeMatch.inGame.playerHealth"),
                    )}
                    {gameState.player.hasShield && (
                      <Text
                        text={`🛡️ ${t("runeMatch.inGame.shield")}`}
                        x={10}
                        y={245}
                        width={layout.sidebarWidth - 20}
                        fontSize={12}
                    ...
                    {gameState.currentStreak > 0 && (
                      <Text
                        text={`🔥 ${t("runeMatch.inGame.combo")} x${gameState.currentStreak}`}
                        x={10}
                        y={260}
                        width={layout.sidebarWidth - 20}
                        fontSize={14}
                    ...
                    {/* Skills Header */}
                    <Text
                      text={t("runeMatch.inGame.skills")}
                      x={10}
                      y={290}
                      width={layout.sidebarWidth - 20}
                      fontSize={10}
                  ...
                  row.map((rune, c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {rune.type === "vocabulary" &&
                          (() => {
                            const textW = runeSize - 8;
                            const textH = Math.round(runeSize * 0.52);
                            const textX = 4;
                            const textY = Math.round((runeSize - textH) / 2);
                            const fs = calcFontSize(rune.text, layout.cellSize);
                            const isThai = isThaiText(rune.text);
                            return (
                              <>
