...
export default function ConveyorBelt({ y, width, layout }: ConveyorBeltProps) {
  ...
  const discardIngredient = usePotionRushStore(
    (state) => state.discardIngredient,
  );
  const setIngredientDragging = usePotionRushStore(
    (state) => state.setIngredientDragging,
  );
...
function IngredientItem({
  ...
  return (
    ...
    >
      {img ? (
        <KonvaImage image={img} x={-40} y={-40} width={80} height={80} />
      ) : (
