...
const renderParticle = (effect: PotionRushEffect, index: number) => {
  ...
  const { angle, speed, size, color, rotation } = getParticleSpec(
    effect,
    index,
  );
...
export default function PotionRushEffectsLayer() {
  ...
  return (
    <Group>
      {effects.map((effect) => (
        <Group key={effect.id}>
          {Array.from({ length: EFFECT_CONFIG[effect.type].count }).map(
            (_, index) => renderParticle(effect, index),
          )}
        </Group>
      ))}
    </Group>
