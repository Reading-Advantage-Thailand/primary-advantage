...
export function GameEndScreen({
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/85 p-6 text-white backdrop-blur-sm"
    >
      <motion.div
      ...
      >
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {statCards.map((stat, index) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-center gap-1 text-xs font-bold tracking-wider text-slate-400 uppercase">
          ...
          <div
            className={`rounded-2xl border p-4 text-center sm:col-span-2 ${statusStyle.xpShell}`}
          >
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <button
            onClick={onRestart}
            className="flex-1 rounded-2xl bg-white py-4 font-black tracking-widest text-slate-950 uppercase shadow-lg transition-all hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 focus-visible:outline-none"
          >
          ...
          {onExit ? (
            <button
              onClick={onExit}
              className="flex-1 rounded-2xl border border-white/10 bg-slate-800/70 py-4 font-bold tracking-widest text-white uppercase transition-all hover:bg-slate-800 focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 focus-visible:outline-none"
            >
              Exit
            </button>
          ) : null}
