...
export function VirtualDPad({ onInput }: VirtualDPadProps) {
  ...
  return (
    <div
      className="relative h-32 w-32 touch-none rounded-full border-2 border-white/10 bg-white/5 shadow-inner select-none"
    ...
    >
      ...
      {/* Thumb Stick */}
      <div
        className={`pointer-events-none absolute top-1/2 left-1/2 -mt-6 -ml-6 h-12 w-12 rounded-full border-2 border-white/30 bg-white/20 shadow-xl transition-transform duration-75 ${active ? "scale-110 opacity-100" : "scale-100 opacity-50"}`}
        style={{
      ...
      {/* Directional Marks */}
      <div className="absolute top-2 left-1/2 -ml-0.5 h-2 w-1 rounded-full bg-white/20" />
      <div className="absolute bottom-2 left-1/2 -ml-0.5 h-2 w-1 rounded-full bg-white/20" />
      <div className="absolute top-1/2 left-2 -mt-0.5 h-1 w-2 rounded-full bg-white/20" />
      <div className="absolute top-1/2 right-2 -mt-0.5 h-1 w-2 rounded-full bg-white/20" />
    </div>
  );
}
