...
export default function Reports() {
  ...
  const columns: ColumnDef<StudentData>[] = [
    ...
    },
    {
      accessorKey: "xp",
      header: () => {
        return <div className="text-center">XP</div>;
      },
      cell: ({ row }) => (
  ...
  const fetchXpPerStudents = async (classId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/classroom/xp-per-students/${classId}`,
        {
          method: "GET",
        },
      );
      ...
      <div className="grid grid-cols-2 items-end">
        ...
        {/* {studentInClass && (
            <div className="flex justify-end">
              <Card className="flex w-[50%] items-center justify-center">
                <CardContent className="mt-4">
                  {trp("averageLevel")}
                  <span className="ml-2 text-xl">{averageLevel.toFixed(2)}</span>
                </CardContent>
              </Card>
            </div>
          )} */}
